...
def run():
    ...
    #   |'FormatData' >> beam.Map(lambda data: format_data(data))
      |'LoadDataInBigQuery' >> beam.Map(lambda main_data: load_bigquery(main_data))
    )
    p.run()
